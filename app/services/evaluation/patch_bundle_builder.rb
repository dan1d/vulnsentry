...
module Evaluation
  class PatchBundleBuilder
    ...
    def resolve_and_update!(bundle, branch_target, bundled_gems_content, entry)
      ...
      begin
        # For Gemfile.lock projects, we use bundle update at PR time
        # For bundled_gems, we validate the text-based bump now
        if project&.file_type == "gemfile_lock"
          diff_preview = gemfile_lock_diff_preview(entry.name, entry.version, target)
        else
          ...
          diff_preview = proposed_diff(bumper)
        end
    ...
    def bump_version_for_project(project:, content:, gem_name:, target_version:)
      # Only used for bundled_gems projects
    ...
    # For Gemfile.lock, we show a preview message since actual diff is generated by bundle update
    def gemfile_lock_diff_preview(gem_name, current_version, target_version)
      <<~PREVIEW
        # Gemfile.lock update via `bundle update #{gem_name} --conservative`
        #
        # Expected change:
        -    #{gem_name} (#{current_version})
        +    #{gem_name} (#{target_version})
        #
        # Note: Actual diff may include platform additions and transitive dependency updates.
      PREVIEW
    end
