...
RSpec.describe Evaluation::CandidateBumpBuilder do
  ...
  it "fails closed with blocked_ambiguous when RubyGems is down" do
    branch = create(:branch_target, name: "master", enabled: true, maintenance_status: "normal")

    advisory = Advisory.create!(
      fingerprint: "osv:OSV-TEST-2",
      gem_name: "rexml",
      source: "osv",
      raw: {
        "affected" => [
    ...
    allow_any_instance_of(RubyLang::SecurityAdvisoryResolver)
      .to receive(:resolve_fixed_version)
      .and_return("3.4.5")
    ...
    bump = CandidateBump.find_by!(advisory: advisory, branch_target: branch)
    expect(bump.state).to eq("blocked_ambiguous")
    expect(bump.blocked_reason).to include("ResolutionError")
  end
end
